...
{//leaf:UIImageView,父类:UIView(UIActivityIndicatorView),root:ViewController
    ...
    NSLog(@"result:%d",isSelected && [views indexOfObject:root] != NSNotFound);
...
{//filter有_name,_predicate,_index,
    MPObjectFilter *filter;
    if ([_scanner scanCharactersFromSet:_separatorChars intoString:nil]) {//第一段为@"/"
        NSString *name;
        filter = [[MPObjectFilter alloc] init];
        if ([_scanner scanCharactersFromSet:_classAndPropertyChars intoString:&name]) {//第二段为字母数字构成的名称
            filter.name = name;
        } else {
            filter.name = @"*";
        }
        if ([_scanner scanCharactersFromSet:_flagStartChar intoString:nil]) {//如果第二段为@"("
            NSString *flags;
            [_scanner scanUpToCharactersFromSet:_flagEndChar intoString:&flags];//截取()内容
            for (NSString *flag in[flags componentsSeparatedByString:@"|"]) {//以|分割截取内容
        ...
        if ([_scanner scanCharactersFromSet:_predicateStartChar intoString:nil])//如果第二段以@"["开头
        {
            ...
            NSInteger index = 0;
            if ([_scanner scanInteger:&index] && [_scanner scanCharactersFromSet:_predicateEndChar intoString:nil]) {//如果数字开头，作为index存入
    ...
    for (NSObject *parent in parents) {
        if ([parent isKindOfClass:[UIView class]]) {//对一些UIView对象进行操作
            ...
            if ((index < 0 || ((NSUInteger)index < [siblings count] && siblings[(NSUInteger)index] == view))
                && (numSiblings < 0 || [siblings count] == (NSUInteger)numSiblings)) {//<0发生条件
                return YES;
            }
        }
    }
...
- (NSArray *)getParentsOfObject:(NSObject *)obj//UIView
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    if ([obj isKindOfClass:[UIView class]]) {//获得此对象的视图
        if ([(UIView *)obj superview]) {
            [result addObject:[(UIView *)obj superview]];
        }
        // For UIView, nextResponder should be its controller or its  .
        if ([(UIView *)obj nextResponder] && [(UIView *)obj nextResponder] != [(UIView *)obj superview]) {//nextResponder != superView
...
{//obj:UIViewController,class:UIView
    ...
    NSLog(@"obj class : %hhd",[obj isKindOfClass:[UIWindow class]]);
//    NSLog(@"rootViewController class:%@",((UIWindow *)obj).rootViewController);
    
    if ([obj isKindOfClass:[UIWindow class]] && [((UIWindow *)obj).rootViewController isKindOfClass:class]) {//obj:UIWindow且obj.rootViewController:UIView
